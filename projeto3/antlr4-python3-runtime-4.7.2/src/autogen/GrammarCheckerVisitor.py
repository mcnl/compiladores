...
class GrammarCheckerVisitor(ParseTreeVisitor):
    ...
    def visitVariable_assignment(self, ctx:GrammarParser.Variable_assignmentContext):
        ...
        if op == '++' or op == '--':
            if(op == '++'):
                cte_value = eval(str(cte_value) +'+'+ '1')
            else:
                cte_value = eval(str(cte_value) +'-'+ '1')
            pass
        else:
            ...
            if expr_type == Type.VOID:
                ...
                print("WARNING: possible loss of information assigning float expression to int variable '" + name + "' in line " + str(token.line) + " and column " + str(token.column))
                if cte_value is not None and expr_cte_value is not None:
                    cte_value = int(eval('{0}{1}'.format(cte_value, str(op).replace('=',str(expr_cte_value)) )))
            else:
                if cte_value is not None and expr_cte_value is not None:
                    cte_value = eval('{0}{1}'.format(cte_value, str(op).replace('=',str(expr_cte_value)) ))
        if ctx.identifier() != None:
    ...
    def visitExpression(self, ctx:GrammarParser.ExpressionContext):
        ...
        if len(ctx.expression()) == 0:
            ...
            if ctx.integer() != None:
                tyype = Type.INT
                cte_value = int(str(ctx.integer().INTEGER()))

            elif ctx.floating() != None:
                tyype = Type.FLOAT
                cte_value = float(str(ctx.floating().FLOATING()))

            elif ctx.string() != None:
                tyype = Type.STRING
                cte_value = str(ctx.string().STRING())
                ...
                try:
                    ...
                    print("ERROR: undefined array '" + name + "' in line " + str(token.line) + " and column " + str(token.column))
                array_index = self.visit(ctx.array())
                if(int(str(array_index))>int(str(array_length))-1):
                    token = ctx.array().identifier().IDENTIFIER().getPayload()
                    print("ERROR: Index value out of range of array '" + name + "' in line " + str(token.line) + " and column " + str(token.column))
            ...
            if left_cte_value != None and right_cte_value!= None:
                cte_value = eval(str(left_cte_value)+text+str(right_cte_value))
                if cte_value == True:
                    cte_value = 1
                elif cte_value == False:
                    cte_value = 0
